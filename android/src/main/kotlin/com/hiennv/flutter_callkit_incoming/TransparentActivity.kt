...
class TransparentActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // `intent.action!!` used to crash here. Finishing quietly is always
        // better: with no action there is no broadcast worth sending.
        val action = intent.action
        if (action == null) {
            Log.w(TAG, "Intent action is null, finishing activity")
            finish()
            return
        }
